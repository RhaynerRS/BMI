...
function BF() {
  ...
  if (alt.indexOf('.' || ',') > -1) {
    ...
    bf = (495 / (1.0324 - 0.19077 * Math.log10(waist - neck) + 0.15456 * Math.log10(altura * 100))) - 450
    result.innerHTML = ("Sua BF é de: " + bf.toFixed(2) + '%')
  }
...
function TMB() {
  ...
  if (sexo == "masculino") {
    if (alt.indexOf('.' || ',') > -1) {
      ...
      tmb = (66 + (13.8 * peso) + (5 * (altura * 100))) - (6.8 * idade)
    }
  }
  else {
    if (alt.indexOf('.' || ',') > -1) {
      tmb = (655 + (9.6 * peso) + (1.8 * (alt * 100))) - (4.7 * idade)
    }
    else {
  ...
  result.innerHTML = ("Sua TMB é de: " + tmb.toFixed(2))
